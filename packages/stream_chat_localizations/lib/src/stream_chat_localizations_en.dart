...
class StreamChatLocalizationsEn extends GlobalStreamChatLocalizations {
  ...
  String pinnedByUserText({
    ...
    if (pinnedByCurrentUser) return 'Marked by You';
    return 'Marked by ${pinnedBy.name}';
  }
  ...
  String togglePinUnpinText({required bool pinned}) {
    if (pinned) return 'Unmark message';
    return 'Mark message';
  }
  ...
  String sentAtText({required DateTime date, required DateTime time}) =>
      'Sent ${_getDay(date)} at ${Jiffy.parseFromDateTime(time.toLocal()).format(
        pattern: 'HH:mm',
      )}';
